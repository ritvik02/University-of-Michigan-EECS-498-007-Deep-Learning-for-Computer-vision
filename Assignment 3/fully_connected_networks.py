...
class Linear(object):
  ...
  def forward(x, w, b):
    ...
    N = x.shape[0]
    transform = x.reshape((N, -1)) # tr = torch.flatten(x, start_dim=1)
  ...
  def backward(dout, cache):
    ...
    '''alternate method
    N = x.shape[0]
    dx_raw = torch.mm(dout, w.t()) # it is N * D i.e 10 * 6
    dx = dx_raw.reshape(x.shape)
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss += ((self.params['W1']*self.params['W1']).sum()+(self.params['W2']*self.params['W2']).sum())*self.reg #adding regularisation to the loss
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    '''alternate method 1
    self.params['W1'] = torch.zeros(input_dim, hidden_dims[0], dtype=dtype,device = device)
    self.params['W1'] += weight_scale*torch.randn(input_dim, hidden_dims[0], dtype=dtype,device= device)
    self.params['b1'] = torch.zeros(hidden_dims, dtype=dtype,device= device)
    #hidden layers 
    for i in range(0, len(hidden_dims)):
      self.params['W'+str(i+2)] = torch.zeros(hidden_dims[i],hidden_dims[i+1], dtype=dtype,device= device)
      self.params['W'+str(i+2)] += weight_scale*torch.randn(hidden_dims[i],hidden_dims[i], dtype=dtype,device= device)
      self.params['b'+str(i+2)] = torch.zeros(hidden_dims[i+1], dtype=dtype,device= device)
    #final layers 
    self.params["W"+ str(self.num_layers)] = weight_scale * torch.randn(hidden_dims[-1], num_classes, dtype=dtype, device=device)
    self.params["b"+ str(self.num_layers)] = torch.zeros(num_classes, dtype=dtype, device=device)'''
