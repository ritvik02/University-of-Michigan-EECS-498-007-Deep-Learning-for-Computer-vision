  ...
  def forward(x, w, b, conv_param):
    ...
    for n in range(N): #each image
      for f in range(F): #each filter
        for height in range(H_out): #row
          for width in range(W_out): #column
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    # print(in_dim)
    # print(H//2)
    in_dim = num_filters*H_out*W_out
    ...
    for l in range(self.num_layers - 1):
      ...
      if l in max_pool_set:
        ...
        if self.batchnorm:
          ...
          # print('idx', p_idx)
    # print('len', len(self.max_pools))
    # assert(p_idx == len(self.max_pools))
