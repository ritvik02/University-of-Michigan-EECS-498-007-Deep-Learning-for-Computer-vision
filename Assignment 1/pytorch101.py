...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  #x=torch.arange(start+5,stop,10, dtype=torch.float64)
...
def slice_assignment_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x[:4, :6] = torch.tensor([[0, 1, 2, 2, 2, 2],[0, 1, 2, 2, 2, 2],
  [3, 4, 3, 4, 5, 5],
  [3, 4, 3, 4, 5, 5]])
  '''alternate method 1 
  x[:2,:1] = 0
...
def count_negative_entries(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  num_neg=x[x<0].shape[0] # or x[x<0].shape  
...
def make_one_hot(x):
  ...
  y[torch.arange(y.shape[0]), x] = torch.tensor([1. for _ in range(y.shape[0])])'''
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  y[range(y.shape[0]),a] = 0 #y[[0,1],[0,2]] = 0
  ...
  '''alternate method 2
  row_idx = torch.arange(y.shape[0])
  _, col_idx = y.min(dim=1)
  y[row_idx, col_idx] = torch.zeros(y.shape[0], dtype=torch.int64)'''
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  z =torch.zeros(x.shape[0],x.shape[1],y.shape[2]) #z1 = torch.empty(B, N, P)
  if use_loop :
...
def normalize_columns(x):
  ...
  '''alternate method
  column_mean = torch.mean(y, dim=0)
  column_std = torch.std(y, dim=0)
  y = torch.div(torch.sub(y, column_mean), column_std)'''
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
