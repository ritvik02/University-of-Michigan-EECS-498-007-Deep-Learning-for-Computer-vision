...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      tr_flat = x_train[i].view(-1)
      te_flat = x_test[j].view(-1)
      sub = torch.sub(tr_flat, te_flat)
      dists[i][j] = torch.dot(sub, sub)'''
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Initialize dists to be a tensor of shape (num_train, num_test) with the
  # same datatype and device as x_train
  tr_flat = torch.flatten(x_train, start_dim=1)
  te_flat = torch.flatten(x_test, start_dim=1)
  num_train = x_train.shape[0]
  num_test = x_test.shape[0]
  dists = x_train.new_zeros(num_train, num_test)
  ...
  for i in range(num_train):
    dists[i] = ((tr_flat[i]-te_flat)**2).sum(dim=1)**(1/2)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  dists = (-2 * torch.mm(tr_flat, te_flat.t()) + torch.sum(te_flat**2, dim=1) + torch.sum(tr_flat**2, dim=1).view(-1, 1))**(1/2)
  #dists = ((A**2).sum(dim = 1).reshape(-1,1) -2 * torch.mm(A, B) + (B**2).sum(dim = 1).reshape(1,-1))**(1/2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  m = torch.zeros(k,dists.shape[1])
  values, indices = torch.topk(dists, k,dim=0, largest = False)
  for i in range(k):
    m[:,i] = y_train[indices[:,i]] 
  y_pred,_= torch.mode(m,dim=0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
